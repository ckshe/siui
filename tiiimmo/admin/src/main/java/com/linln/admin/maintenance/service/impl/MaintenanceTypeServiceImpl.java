...
import com.linln.admin.maintenance.repository.MaintenanceTypeRepository;
...
public class MaintenanceTypeServiceImpl implements MaintenanceTypeService {
     ...
     */
    @Override
    public Page<MaintenanceType> getPageList(Example<MaintenanceType> example) {
        // 创建分页对象
        PageRequest page = PageSort.pageRequest();
        return maintenanceTypeRepository.findAll(example, page);
    }
    ...
    public Boolean updateStatus(StatusEnum statusEnum, List<Long> idList) {
        return maintenanceTypeRepository.updateStatus(statusEnum.getCode(), idList) > 0;
    }
}
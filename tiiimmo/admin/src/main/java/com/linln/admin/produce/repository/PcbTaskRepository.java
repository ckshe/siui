...
public interface PcbTaskRepository extends BaseRepository<PcbTask, Long> {
    ...
    @Query(value = "SELECT * FROM produce_pcb_task where (produce_plan_complete_date >= ?1 AND produce_plan_complete_date <= ?2) or (produce_date >= ?1 AND produce_complete_date <= ?2) order by produce_complete_date desc",nativeQuery = true)

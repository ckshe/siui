...
public interface UserDeviceHistoryRepository extends BaseRepository<UserDeviceHistory, Long> {
    ...
    //根据日期员工设备工序计划查找上下机记录
    @Query(value = "SELECT * FROM produce_user_device_history WHERE process_task_code=?1 AND  CONVERT(varchar(100), up_time, 23) = ?2 AND device_code = ?3 and user_id = ?4  AND down_time is  null",nativeQuery = true)
